...
const Home = () => {
  ...
  const fetchUserProfile = useCallback(async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('profiles').select('game_type').eq('id', user.id).single();
      if (!error && data) {
  ...
  const fetchTournaments = useCallback(async () => {
    try {
      let query = supabase.from('tournaments').select('id, title, game_type, entry_fee, prize_pool, per_kill_prize, total_slots, filled_slots, scheduled_at, image_url').eq('status', 'upcoming').order('scheduled_at', {
        ascending: true
      }).limit(2);
      ...
      <section className="relative h-[300px] overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg mx-auto px-4 h-full flex flex-col justify-end pb-8">
          <h2 className="text-3xl font-bold mb-2 text-white drop-shadow-lg">
            Join Epic Tournaments and win money    
          </h2>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div>
          ...
          <div className="space-y-4">
            {tournaments.length === 0 ? <Card className="glass border-border p-8">
                <p className="text-center text-muted-foreground">
                  {userGameType ? `No ${userGameType} tournaments available at the moment` : 'No tournaments available. Please set your game type in your profile.'}
                </p>
              </Card> : tournaments.map(tournament => {
            const gameImage = tournament.game_type?.toUpperCase() === 'FF' ? ffTournament : bgmiTournament;
            return <Link key={tournament.id} to={`/tournament/${tournament.id}`}>
                    <Card className="glass border-border overflow-hidden hover:border-primary/50 transition-all group">
                    <div className="relative">
                      <img src={tournament.image_url || gameImage} alt={tournament.title} className="w-full h-32 object-cover group-hover:scale-105 transition-transform duration-300" />
                    ...
                    <div className="p-4">
                      ...
                      {tournament.per_kill_prize > 0 && <div className="mb-4 p-2 bg-green-500/10 border border-green-500/30 rounded-lg">
